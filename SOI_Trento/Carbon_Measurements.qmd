...
```{r}
medium <- read.csv(file = "C:/Users/Utente/Documents/SOI_Trento/silicon_linearization_med_gain_C.csv")
low <- read.csv(file = "C:/Users/Utente/Documents/SOI_Trento/silicon_linearization_low_gain_C.csv")
```
...
list_files <- list.files("~/SOI_Trento/MedAustron/",pattern = ".Spe")

path <- glue("~/SOI_Trento/MedAustron/",list_files[1])
df <- fread(path)
...
for (file in list_files[-1]) {
  ...
  path <- glue("~/SOI_Trento/MedAustron/",file)
  df <- fread(path)
